...
namespace objl
{
		...
		bool LoadFile(std::string Path)
		{
			...
			while (std::getline(file, curline))
			{
				...
				if (algorithm::firstToken(curline) == "o" || algorithm::firstToken(curline) == "g" || curline[0] == 'g')
				{
					...
					{
						// Generate the meshes to put into the array
			...
			for (int i = 0; i < MeshMatNames.size(); i++)
			{
				...
				// when found copy material variables into meshes material
	...
	private:
			...
			std::string icurline)
		{
			...
			// best they get for not compiling a meshes with normals
			if (noNormal)
			{
